...
"该企业在认证的“[平台名称，如微信小程序/淘宝/京东/拼多多]”店铺内销售商品“[商品名称]”（商品/店铺链接：[链接/路径]），其商品详情页/宣传页介绍内容属[广告类型]，涉嫌违反[列举法律名称]发布违法广告。该商品页面内容存在[具体违规行为描述]，明确表述[引用违规原文片段]，违反《[法律名称]》第[几]条禁止性规定；[如果是医疗广告且未审查/违规]同时该[类型]广告未经[相关部门]审查批准/擅自发布，违反《[法律名称]》第[几]条审查程序要求。[如果有传播数据，必须在此插入一句：经查，该商品页面显示已售/销量为[具体数字]，传播范围较广/影响较大。]"
