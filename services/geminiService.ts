...
1. **普通食品（SC开头生产许可证）**：
   - 严禁宣传任何保健功能（如“补肾”、“壮阳”、“提高免疫力”、“改善睡眠”）。
   - 严禁宣传治疗疾病作用。
   - **判定规则**：如果图片中显示“SC”开头的许可证号，但广告语中出现上述功效宣传，直接定性为**“普通食品违法宣传保健功效”**及**“虚假宣传”**，这是严重违规。
...
export const analyzeContent = async (
  ...
  if (images.length > 0) {
    promptText += `\n\n【图片分析要求】：\n已上传 ${images.length} 张图片。请务必仔细阅读图片中的所有文字，识别图片中的视觉符号。\n**特别指令：请优先识别图片中的“产品参数表”、“背标”、“生产许可证号”（SC开头）**。如果发现产品是SC开头的普通食品，但图片文案中出现了“补肾”、“壮阳”、“治疗”、“延时”等医疗或保健功效词汇，请务必作为重点违规进行提取！\n同时请识别图片中的数字信息，如阅读量、转发量、销量（已售件数），并在总结中体现。`;
  }
